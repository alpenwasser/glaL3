...
R_tot = lambda w: -(w * mu0 * 2 * pi * r0 * N0**2) / l * im(besselj(1,k(w) * r0) / (k(w) * besselj(0,k(w) * r0))) + R_0
...
Rnum   = Rufunc(frequency_vector)                    # numerical results for R
...
#subplot(2,1,1)
plot(frequency_vector,Rnum,color='blue',label='Fitfunktion')
